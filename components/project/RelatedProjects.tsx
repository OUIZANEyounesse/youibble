    ...
    <section className="flex flex-col mt-32 w-full">
      ...
      <div className="related_projects-grid">
        {filteredProjects?.map(({node}: {node: ProjectInterface}) =>(
            <div className="flexCenter related_project-card drop_shadow-card">
                <Link href={`/project/${node?.id}`} className="flexCenter group relative w-full h-full">
                    <Image src={node?.image} width={424} height={314} alt="project image" className="w-full h-full object-cover rounded-2xl"/>
